...
contract surface, consensus boundaries, and deterministic standardization
...
def _load_contract():
    _install_stub()
    spec = importlib.util.spec_from_file_location("quorum_contract", CONTRACT)
...
def _ast_checks(source: str) -> list[str]:
    ...
    required_classes = {"Quorum", "Rulebook", "Rule", "Relation", "IQuorum"}
    required_methods = {
        ...
        "get_standard", "blocking_reason", "is_consistent", "is_consistent_for",
        "current_standard_hash",
    }
    ...
    assert "QUORUM / INDEPENDENTLY NORMALIZE RULE" in source
    assert "QUORUM / COMPARE INDEPENDENT NORMALIZATIONS" in source
    assert "QUORUM / INDEPENDENTLY CLASSIFY RULE RELATION" in source
    ...
    assert "standard_hash" in source and "is_consistent_for" in source
    checks.append("cross-contract standard pinning surface present")
    return checks


def _helper_checks(c) -> list[str]:
    checks = []
    sem = c.standard_semantics({
        "atomic": True,
        "modality": "PROHIBIT",
    ...
    checks.append("clear atomic semantics standardize and hash")

    ambiguous = c.standard_semantics({
        "atomic": False,
        "modality": "REQUIRE",
        "actor": "council",
        "action": "act",
    ...
    checks.append("conflict relation standardizes into bounded enums")

    nonconflict = c.standard_relation({
        "relation": "COMPATIBLE",
        "conflict_type": "MODAL",
        "overlap": "same domain",
...
def main() -> int:
    ...
    print(f"Quorum offline preflight: {len(checks)}/{len(checks)} checks passed")
    for index, check in enumerate(checks, 1):
        print(f"  {index:02d}. PASS - {check}")
    print("\nGenVM consensus/runtime behavior is covered separately by tests/test_quorum.py.")
    return 0
