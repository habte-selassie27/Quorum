...
Quorum therefore uses two bounded consensus calls with `gl.vm.run_nondet_unsafe`.
...
The leader receives immutable rulebook purpose and immutable candidate rule text, then proposes one standard semantic object:
...
Quorum still treats prompt injection as a model-layer risk, which is why outputs are bounded, validator-reviewed, and unable to choose priority.
...
The contract does not silently coerce rejected or malformed outputs into active standard.
